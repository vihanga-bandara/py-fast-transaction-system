...
class AccountService:
    ...
    async def get_all_accounts(self) -> list[Account]:
        accounts = await self._account_repository.get_all()
        return accounts

    async def get_account_by_name(self, account_name: str) -> Account:
        account = await self._account_repository.get_by_name(account_name)
        if not account:
            raise AccountNotFoundError(account_name)
        return account

    async def create_account(self, account_name: str) -> Account:
        return await self._account_repository.create_account(account_name)
